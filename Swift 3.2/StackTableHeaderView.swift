...
class StackTableHeaderView: UIView {
    ...
    private func updateViews(){
        ...
        self.addsubViews(views: vs)
        let height = self.views.reduce(0) { (old, s) in
            return old + s.preferHeaderHeight()
        }
        self.frame = CGRect(x: 0, y: 0, width: self.tableView.frame.width, height: height + CGFloat.leastNormalMagnitude)
    ...
    convenience init(tableView: UITableView) {
        self.init(frame:CGRect(x: 0, y: 0, width: 0, height: CGFloat.leastNormalMagnitude))
